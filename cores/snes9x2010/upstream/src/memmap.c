...
#ifdef PSP
	if (!s9x_stream_buffer && s9x_stream_path && s9x_stream_path[0])
	{
		RFILE *file = filestream_open(s9x_stream_path,
				RETRO_VFS_FILE_ACCESS_READ,
				RETRO_VFS_FILE_ACCESS_HINT_NONE);
		int64_t read;
		if (!file)
			return (0);
		read = filestream_read(file, buffer, (int64_t)maxsize + 0x200);
		filestream_close(file);
		if (read <= 0 || read > UINT32_MAX)
			return (0);
		return HeaderRemove((uint32_t)read, &Memory.HeaderCount, buffer);
	}
#endif
	fp = OPEN_STREAM(0);
	if (!fp)
