...
#else
	/* The Blargg filter table alone consumes 4 MiB of static memory.
	 * It is a presentation extra, so omit it from the PSP-1000 build. */
	snes_ntsc_filter = 0;
#endif
...
bool retro_load_game(const struct retro_game_info *game)
{
	...
	else if (game->path && game->path[0])
		S9xSetStreamPath(game->path);
	else
		return FALSE;
