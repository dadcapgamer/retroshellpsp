...
{
   bool updated = false;
   bool frontend_skip_video = false;
   ...
   /* RetroSuite's PSP audio scheduler may run a short burst of recovery
    * frames before gpSP's own audio-buffer callback reaches its emergency
    * threshold. Honour the standard libretro video-enable bit so those
    * frames skip scanline rendering, not merely the final video callback.
    * This leaves game logic and audio enabled and gives the 333 MHz PSP
    * enough headroom to rebuild the ring during demanding scenes. */
   if (environ_cb(RETRO_ENVIRONMENT_GET_AUDIO_VIDEO_ENABLE,
         &audio_video_enable))
      frontend_skip_video = (audio_video_enable & 1) == 0;
